...
mask = rng_data.choice(np.arange(len(x_grid.flatten())), N)
...
# N, - Beobachtungen (Rauschen hinzufügen: + rng.standard_normal(N)*0.1)
y = nature(X.T)
...
def unpack(vec):
  i = 0
  W1 = vec[i: i + P * H1].reshape(P, H1)
  i += P * H1
  b1 = vec[i: i + H1]
  i += H1
  W2 = vec[i: i + H1 * H2].reshape(H1, H2)
  i += H1 * H2
  b2 = vec[i: i + H2]
  i += H2
  Wout = vec[i: i + H2]
...
# # Autograd-Alternative für schnelle Modelländerungen (tanh, etc...): nächste Zeile und Import ganz oben auskomomentieren
# g_f = egrad(f)
